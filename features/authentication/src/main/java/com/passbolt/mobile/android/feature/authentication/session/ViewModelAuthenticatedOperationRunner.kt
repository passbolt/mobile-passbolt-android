 ...
 */
class ViewModelAuthenticatedOperationRunner(
    private val needAuthenticationRefreshedFlow: MutableSharedFlow<UnauthenticatedReason?>,
    private val authenticationRefreshedFlow: SharedFlow<Unit?>,
) : KoinComponent {
...
/**
 * Runs an operation which requires authentication using ViewModelAuthenticatedOperationRunner
 * @see AuthenticatedOperationRunner
 */
suspend fun <OUTPUT : AuthenticatedUseCaseOutput> AuthenticatedViewModel<*, *>.runAuthenticatedOperation(
    request: suspend () -> OUTPUT,
): OUTPUT =
    ViewModelAuthenticatedOperationRunner(this.needSessionRefreshFlow, this.sessionRefreshedFlow)
        .runOperation(request)
