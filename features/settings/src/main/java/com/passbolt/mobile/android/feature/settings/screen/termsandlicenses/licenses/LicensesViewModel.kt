...
internal class LicensesViewModel(
    private val licensesReader: LicensesReader,
) : SideEffectViewModel<LicensesState, LicensesSideEffect>(LicensesState()),
    KoinComponent {
    init {
        loadInitialValues()
    }

    fun onIntent(intent: LicensesIntent) {
        when (intent) {
            GoBack -> emitSideEffect(NavigateUp)
            is LicensesIntent.GoToLicenseUrl -> emitSideEffect(NavigateToLicenseUrl(intent.licenseUrl))
        }
    }

    private fun loadInitialValues() {
        viewModelScope.launch {
            val licenses = licensesReader.getLicenses()
            updateViewState {
                copy(openSourceLicensesModel = licenses)
            }
        }
    }
}
