...
internal class TransferAccountSummaryViewModel :
    ...
    fun onIntent(intent: TransferAccountSummaryIntent) {
        when (intent) {
            is Initialize ->
                updateViewState {
                    copy(statusType = intent.statusType)
                }
            PrimaryAction -> emitSideEffect(NavigateToMyAccount)
            GoBack -> emitSideEffect(NavigateToTransferAccountStart)
            TryAgain -> emitSideEffect(NavigateToTransferAccountStart)
        }
    }
}
