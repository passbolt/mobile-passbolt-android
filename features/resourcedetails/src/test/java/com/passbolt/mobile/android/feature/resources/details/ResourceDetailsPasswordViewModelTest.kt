...
class ResourceDetailsPasswordViewModelTest : KoinTest {
    @get:Rule
    val koinTestRule =
        KoinTestRule.create {
    ...
    @Test
    fun `toggle password visibility should show password when hidden`() =
        runTest {
    ...
    @Test
    fun `toggle password visibility should hide password when visible`() =
        runTest {
            val password = "secretPassword123"
            val secretPropertiesActionsInteractor: SecretPropertiesActionsInteractor = get()
            secretPropertiesActionsInteractor.stub {
            ...
            viewModel.viewState.drop(1).test {
                ...
                val state = awaitItem()
                assertThat(state.passwordData.isPasswordVisible).isFalse()
                assertThat(state.passwordData.password).isEmpty()
            }
        }

    @Test
    fun `copy password should emit add to clipboard side effect when rbac allows`() =
        runTest {
            val password = "secretPassword123"
            val secretPropertiesActionsInteractor: SecretPropertiesActionsInteractor = get()
            secretPropertiesActionsInteractor.stub {
    ...
    @Test
    fun `password decryption failure should show error snackbar`() =
        runTest {
            val secretPropertiesActionsInteractor: SecretPropertiesActionsInteractor = get()
            secretPropertiesActionsInteractor.stub {
    ...
    @Test
    fun `password fetch failure should show error snackbar`() =
        runTest {
            val secretPropertiesActionsInteractor: SecretPropertiesActionsInteractor = get()
            secretPropertiesActionsInteractor.stub {
    ...
    @Test
    fun `password eye icon should not be shown when preview disabled by feature flag`() =
        runTest {
    ...
    @Test
    fun `password eye icon should not be shown when preview disabled by rbac`() =
        runTest {
