...
import com.passbolt.mobile.android.ui.NewMetadataKeyToTrust
...
class NewMetadataKeyTrustDialog : DialogFragment(), KoinComponent {
    ...
    private val bundledNewKeyToTrust by lifecycleAwareLazy {
        requireNotNull(
            BundleCompat.getParcelable(requireArguments(), NEW_KEY_TO_TRUST, NewMetadataKeyToTrust::class.java)
        )
    }
    ...
    private fun setupListeners(binding: DialogNewMetadataKeyTrustBinding) {
        with(binding) {
            trustButton.setDebouncingOnClick {
                listener?.trustTheKeyClick(bundledNewKeyToTrust)
                dismiss()
            }
    ...
    interface Listener {
        fun trustTheKeyClick(fingerprint: NewMetadataKeyToTrust)
    }

    companion object {
        private const val NEW_KEY_TO_TRUST = "NEW_KEY_TO_TRUST"

        fun newInstance(newMetadataKeyToTrust: NewMetadataKeyToTrust) =
