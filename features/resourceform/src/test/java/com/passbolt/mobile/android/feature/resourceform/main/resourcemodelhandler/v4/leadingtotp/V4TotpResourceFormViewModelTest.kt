...
class V4TotpResourceFormViewModelTest : KoinTest {
    ...
    fun `edit metadata should not change content type and apply changes`() =
        runTest {
            ...
            viewModel.onIntent(TotpUrlChanged(mockMainUri))
    ...
    fun `add totp should not change content type and apply changes`() =
        runTest {
            ...
            viewModel.onIntent(TotpSecretChanged(mockTotpSecret))
            viewModel.onIntent(
                TotpAdvancedSettingsResult(
                    TotpUiModel(
                        secret = "",
                        issuer = "",
    ...
    fun `add note should change content type and apply changes`() =
        runTest {
            ...
            viewModel.onIntent(NameTextChanged(mockName))
            viewModel.onIntent(TotpUrlChanged(mockUrl))
            viewModel.onIntent(TotpSecretChanged(mockTotpSecret))
            viewModel.onIntent(
                TotpAdvancedSettingsResult(
                    TotpUiModel(
    ...
    fun `scan totp should apply model changes`() =
        runTest {
            ...
            viewModel.onIntent(ScanOtpResult(isManualCreationChosen = false, scannedTotp = scannedTotp))
    ...
    @Test
    fun `add metadata description to totp description should not be possible`() =
        runTest {
            val mockName = "test name"
