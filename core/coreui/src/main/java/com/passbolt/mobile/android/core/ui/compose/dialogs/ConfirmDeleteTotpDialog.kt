...
@Composable
fun ConfirmTotpDeleteAlertDialog(
    isVisible: Boolean,
    onConfirm: () -> Unit,
    onDismiss: () -> Unit,
) {
    if (isVisible) {
        AlertDialog(
            ...
            text = { Text(stringResource(LocalizationR.string.otp_delete_confirmation)) },
            confirmButton = {
                TextButton(onClick = onConfirm) {
                    Text(stringResource(LocalizationR.string.otp_delete_totp))
                }
            },
            dismissButton = {
