...
fun SetupExitConfirmationDialog(
    ...
    if (isVisible) {
        AlertDialog(
            ...
            text = { Text(stringResource(LocalizationR.string.scan_qr_exit_confirmation_dialog_message)) },
            confirmButton = {
            ...
            dismissButton = {
                TextButton(onClick = onStopSetup) {
                    Text(stringResource(LocalizationR.string.stop_scanning))
                }
            },
        )
    }
