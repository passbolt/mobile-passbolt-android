...
import com.passbolt.mobile.android.core.permissions.permissions.validation.HasAtLeastOneOwnerPermission
...
class ResourcePermissionsPresenter(
    ...
    override fun argsReceived(permissionsItem: PermissionsItem, id: String, mode: ResourcePermissionsMode) {
        this.mode = mode
        this.id = id
    ...
    override fun actionButtonClick() {
        when (mode) {
            ResourcePermissionsMode.VIEW -> view?.navigateToSelfWithMode(id, ResourcePermissionsMode.EDIT)
    ...
    private fun shareResource() {
        ...
        scope.launch {
            when (runAuthenticatedOperation(needSessionRefreshFlow, sessionRefreshedFlow) {
                shareInteractor.simulateAndShare(id, recipients)
            }) {
