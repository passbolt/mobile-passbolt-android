...
class RemoveLocalFoldersWithUpdateStateUseCaseTest : KoinTest {
    private val useCase: RemoveLocalFoldersWithUpdateStateUseCase by inject()
    ...
    fun `should call dao with correct update state`() =
        runTest {
            useCase.execute(RemoveLocalFoldersWithUpdateStateUseCase.Input(FolderUpdateState.PENDING))

            verify(get<FoldersDao>()).removeWithUpdateState(FolderUpdateState.PENDING)
        }
