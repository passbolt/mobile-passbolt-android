...
class RootRelativeJsonPathMetadataCustomFieldsDelegate(
    private val jsonPath: String,
) : ReadWriteProperty<JsonModel, MetadataCustomFieldsModel?>,
    KoinComponent {
    ...
    override fun getValue(
        thisRef: JsonModel,
        property: KProperty<*>,
    ): MetadataCustomFieldsModel? =
        jsonPathsOps.readOrNull(thisRef) { "$.$jsonPath" }?.let {
            gson.fromJson(it, MetadataCustomFieldsModel::class.java)
        }

    override fun setValue(
        thisRef: JsonModel,
        property: KProperty<*>,
        value: MetadataCustomFieldsModel?,
    ) {
        if (value == null) {
